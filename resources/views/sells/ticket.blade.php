    ...
    <table width="100%">
      <tbody>
        <tr>
          ...
          <td width="70%">
            <h2>Recibo de Venta</h2><br>
            <strong>Fecha:</strong> <?php echo date('d/M/Y');?><br>
            <strong>Recibo Número:</strong> <?=$sale->id?><br>
            
            <strong>Fecha del Servicio:</strong> <?php echo date_format($sale->created_at,"Y-m-d H:i:s") ?><br>
          </td>
        </tr>
        <tr>
          <td colspan="2">&nbsp;</td>
        ...
        <tr>
          <td colspan="2">
            <div class="center-justified">
              ...
              <strong>Total:</strong> <?php setlocale(LC_MONETARY, 'en_US'); echo '$'.money_format('%(#10n', $sale->total).' M.N.' ?>
            </div>
          </td>
        </tr>
      </tbody>
          ...
          <td class="pad-left">
            <ul>
              <?php foreach($products as $p):?>
                <li><?= isset($names[$p->product]) ? $names[$p->product].' '.'$'.money_format('%(#10n', $p->product_price).' M.N.' : 'No'?></li>
              <?php endforeach ?>
            </ul>
          </td>
          <td class="center">Impuesto (16%)</td>
          <td class="right-center"><?php setlocale(LC_MONETARY, 'en_US'); echo '$'.money_format('%(#10n', $sale->total*0.16).' M.N.' ?></td>
        </tr>
        <tr class="border-right">
          <td class="pad-left">&nbsp;</td>
          <td class="right border-top">Subtotal</td>
          <td class="right border-top"><?php setlocale(LC_MONETARY, 'en_US'); echo '$'.money_format('%(#10n', $sale->total).' M.N.' ?></td>
        </tr>
        <tr class="border-right">
          <td class="pad-left">&nbsp;</td>
          <td class="right border-top">Total</td>
          <td class="right border-top"><?php setlocale(LC_MONETARY, 'en_US'); echo '$'.money_format('%(#10n', ($sale->total + ($sale->total * 0.16))).' M.N.' ?></td>
        </tr>
      </tbody>
    </table>
    <p>&nbsp;</p>
